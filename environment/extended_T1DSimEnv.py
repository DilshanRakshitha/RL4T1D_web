...
class T1DSimEnv(gym.Env):
    ...
    def __init__(self, patient_name=None, reward_fun=None, seed=None, args=None, env_type=None, custom_scenario_params=None):
        ...
        self.custom_scenario_params = custom_scenario_params
        if env_type is None:
            env_type = 'training'
    ...
    def _step(self, action):  # This gym only controls basal insulin
        ...
        if self.reward_fun is None:
            ...
            obs, _ = self.state_space.update(cgm=state.CGM, ins=action, meal=0)
    ...
    def calibration_process(self):
        ...
        for t in range(0, self.args.calibration):  # open-loop simulation for calibration period.
            act = Action(basal=self.std_basal, bolus=0)
            future_carb, remaining_time, day_hour, day_min, meal_type = self.announce_meal(meal_announce=None)

            state, reward, is_done, info = self.env.step(act)
            self.cur_state, _ = self.state_space.update(cgm=state.CGM, ins=self.std_basal, meal=0)
    ...
    def _create_env_from_random_state(self):
        ...
        hour = 0  #always start at midnight
        ...
        if self.custom_scenario_params and 'scenario_list' in self.custom_scenario_params:
            # User-provided scenario
            # Ensure start_time in custom_scenario_params matches or is used appropriately
            scenario_start_time = self.custom_scenario_params.get('start_time', start_time)
            scenario = CustomScenario(start_time=scenario_start_time, scenario=self.custom_scenario_params['scenario_list'])
            if self.args.verbose:
                print(f"Using CustomScenario for {self.patient_name} with meals: {self.custom_scenario_params['scenario_list']}")
        else:
            # Fallback to RandomScenario (existing logic)
            scenario = RandomScenario(start_time=start_time, seed=seed3, opt=self.args, env_type=self.env_type)
    ...
    @property
    def std_basal_rate(self):
        return self.std_basal
