...
def run_simulation_for_agent(model_filename_template, meal_data_for_sim, agent_type_suffix):
    ...
    if not os.path.exists(actor_path): logging.error(f"Actor model not found: {actor_path}"); return {"error": f"Actor model not found: {actor_path}"}
    if not os.path.exists(critic_path): logging.error(f"Critic model not found: {critic_path}"); return {"error": f"Critic model not found: {critic_path}"}
    ...
    try:
        ...
        if not os.path.exists(log_file_path):
            ...
            if os.path.exists(worker_args.worker_log_path_base): logging.error(f"Files in {worker_args.worker_log_path_base}: {os.listdir(worker_args.worker_log_path_base)}")
            else: logging.error(f"Log directory {worker_args.worker_log_path_base} does not exist.")
